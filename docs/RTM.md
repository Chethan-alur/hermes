...
| **REQ-FUNC-018** | Reverse-connect transport mode: the **phone dials the Windows client** (which listens), inverting the default phone-as-server topology, so Hermes works over networks that permit only outbound connections from the phone — notably a full-tunnel corporate VPN with stateful client isolation (verified: laptop→phone `:9999` never reaches the phone at any address — VPN pool `10.212.134.150` or NAT `10.141.1.254`; phone→laptop `10.141.1.47` connects, phone NAT'd to `10.141.1.254`) | `android/transport`, `android` (UI), `windows` (tray client) | Reuses every existing `v1/*.json` frame; **only the TCP connection direction is inverted, the data flow (phone→PC transcripts, PC→phone commands) is unchanged** — no new contract | `UT-AND-REVERSEHOSTS-001` | Android `TransportServerService` gains a **Dial** plan alongside **Serve**: when the `reverse_connect` pref is on and ≥1 target host is set, it opens an outbound `Socket` to the configured PC host(s):port and runs the same `handleClientConnection` loop (heartbeat + `activeWriters` + command reader), reconnecting with backoff; mDNS is not advertised in dial mode. Host list parsed by the pure `parseReverseHosts` helper (`UT-AND-REVERSEHOSTS-001`, comma/space/semicolon separated, de-duped). The Windows tray gains a **Listen mode** toggle: `Ensure-Listening` binds a `TcpListener` on `:listenPort` and, on `Pending()`, `AcceptTcpClient` populates the same `$global:tcpClient`/`$global:stream`/`$global:writer` used by the dial path, so `Process-HermesLine`, `Send-HermesCommand`, `Update-TrayIcon`, the overlay and the disconnect handler all work unchanged; config keys `listen` (bool) + `listenPort` (int, 9999). Verification: Android dial-out verified live against a host TCP listener (phone connects over the office VPN as `10.141.1.254`, sends heartbeat, honours `start_listening`); Windows listen-mode UI verified on the host (`pwsh` unavailable under WSL); firewall inbound-allow documented. | Defined |
